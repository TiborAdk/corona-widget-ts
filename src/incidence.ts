...
class IncidenceContainer extends CustomWidgetStack {
    ...
    setFont(font: CustomFont, fontSizeOffset?: number) {
        this.part0.font = font;
        if (fontSizeOffset) this.part1.font = font.newSizedByOffset(fontSizeOffset);
    }

    setFontSmaller(font: CustomFont) {
...
class IncidenceData<T extends MetaData> extends CustomData<IncidenceValue, T> {
    ...
    static async loadFromCache<T extends MetaData>(id: string, typeCheckMeta: (meta: unknown) => meta is T, ...params: any): Promise<DataResponse<IncidenceData<T>> | EmptyResponse> {
    ...
    static calcIncidence<T extends MetaData>(dataObject: IncidenceData<T>, disableLive: boolean = CFG.def.incidenceDisableLive): IncidenceData<T> {
        ...
        const meta = (dataObject.meta as MetaArea);
    ...
    static async loadVaccine(api: RkiService, name?: string): Promise<DataResponse<VaccineData> | EmptyResponse> {
        ...
        const matches = data.data.filter(elem => (elem as any).name === (name ?? 'Deutschland'));
        if (matches.length !== 1) {
    ...
    static isVaccinatedData<T>(data: unknown, fn: (a: unknown) => a is T): data is VaccineData {
        if (data === undefined) {
    ...
    static isVaccineData(data: unknown): data is VaccineData {
        if (data === undefined) {
        ...
        if (!IncidenceData.isVaccinatedData<ApiVaccinatedAtLeastOnce>(data['vaccinatedAtLeastOnce'], RkiService.isApiVaccinatedAtLeastOnce)) {
            console.warn(`isVaccineData: data.vaccinatedAtLeastOnce not of type 'VaccinatedAtLeastOnce'`);
            return false;
        }
        if (!IncidenceData.isVaccinatedData<ApiVaccineFullyVaccinated>(data['fullyVaccinated'], RkiService.isApiVaccineFullyVaccinated)) {
    ...
    static isMetaState(meta: MetaData): meta is MetaState {
    ...
    static isMetaCountry(meta: MetaData): meta is MetaCountry {
        const keys = ['r'];
        const keysNumber = ['EWZ'];
        if (!Helper.keysNumberAreDefined(meta, keysNumber)) {
    ...
    static isMetaArea(meta: MetaData): meta is MetaArea {
...
class Version {
    ...
    constructor(major: number, minor = 0, release = 0, beta = false) {
    ...
    public get arr(): [number, number, number] {
    ...
    static compare(a: Version, b: Version): -1 | 0 | 1 {
        // 0 if a == b
        // -1 if a < b
        // 1 if a > b
    ...
    static fromString(str: string): Version | null {
        const arr = str.split('-');
        let beta = false;
        if (arr.length > 2) {
            return null;
...
class Helper {
    ...
    static keysAreDefined(object: unknown, keys: string[], name?: string, log = true): boolean {
        if (typeof object !== 'object' || object === null) {
            if (log) console.warn(`keysAreDefined: object is not an object or null. (${typeof object}, ${object})`);
            return false;
        }
        for (const key of keys) {
            if (object[key] === undefined) {
                if (log) console.warn(`keysAreDefined: Object is not of type '${name}'. Key '${key}' is missing.`)
                return false;
            }
        }
    ...
    static keysNumberAreDefined(object: unknown, keys: string[], keysOptional: string[] = [], log = true): boolean {
        if (typeof object !== 'object' || object === null) {
            if (log) console.warn(`keysNumberAreDefined: object is not an object or null. (${typeof object}, ${object})`);
            return false;
        }
        ...
        for (const key of keys) {
            ...
            if (value === undefined) {
                if (log) console.warn(`keysNumberAreDefined: required key '${key}' is not defined!`);
                return false;
            }
            if (!Number.isFinite(value)) {
                if (log) console.warn(`keysNumberAreDefined: key '${key}' is not a number. (${typeof value}, ${value})!`);
                return false;
            }
        }
        return true;
    ...
    static migrateConfig1_2_0To1_3(old: ConfigOpt): ConfigOpt1_3 {
        console.log('Migrating config from 1.2.0 to 1.3');
        old.version = undefined;
        return {config: "1.3", ...old};
    }

    static migrateConfigToNewest(old, from: string): ConfigOpt1_3 {
...
class RkiService implements RkiServiceInterface {
    ...
    async casesGer(): Promise<false | IncidenceValue[]> {
        const apiStartDate = Helper.getDateBefore(CFG.def.maxShownDays + 7);
        let urlToday = `${this.rkiApiEndpoint}query?f=json&where=NeuerFall%20IN(1,-1)&returnGeometry=false&geometry=42.000,12.000&geometryType=esriGeometryPoint&inSR=4326&spatialRel=esriSpatialRelWithin&outFields=*&outStatistics=%5B%7B%22statisticType%22%3A%22sum%22,%22onStatisticField%22%3A%22AnzahlFall%22,%22outStatisticFieldName%22%3A%22cases%22%7D%5D&resultType=standard&cacheHint=true`;
        urlToday += `&groupByFieldsForStatistics=MeldeDatum`;
        const urlHistory = `${this.rkiApiEndpoint}query?where=NeuerFall+IN%281%2C0%29+AND+MeldeDatum+%3E%3D+TIMESTAMP+%27${apiStartDate}%27&objectIds=&time=&resultType=standard&outFields=AnzahlFall%2CMeldeDatum&returnIdsOnly=false&returnUniqueIdsOnly=false&returnCountOnly=false&returnDistinctValues=false&cacheHint=false&orderByFields=MeldeDatum&groupByFieldsForStatistics=MeldeDatum&outStatistics=%5B%7B%22statisticType%22%3A%22sum%22%2C%22onStatisticField%22%3A%22AnzahlFall%22%2C%22outStatisticFieldName%22%3A%22cases%22%7D%5D%0D%0A&having=&resultOffset=&resultRecordCount=&sqlFormat=none&f=pjson&token=`;
        const hist = await this.getCases(urlToday, urlHistory);
        return typeof hist === 'boolean' ? hist : IncidenceData.completeHistory(hist, CFG.def.maxShownDays + 7, new Date().setHours(0, 0, 0, 0));
    ...
    async casesState(id: string): Promise<false | IncidenceValue[]> {
        const apiStartDate = Helper.getDateBefore(CFG.def.maxShownDays + 7);
        const urlToday = `${this.rkiApiEndpoint}query?f=json&where=NeuerFall%20IN(1,%20-1)+AND+IdBundesland=${id}&objectIds=&time=&resultType=standard&outFields=&returnIdsOnly=false&returnUniqueIdsOnly=false&returnCountOnly=false&returnDistinctValues=false&cacheHint=false&orderByFields=&groupByFieldsForStatistics=MeldeDatum&outStatistics=%5B%7B%22statisticType%22%3A%22sum%22,%22onStatisticField%22%3A%22AnzahlFall%22,%22outStatisticFieldName%22%3A%22cases%22%7D%5D&having=&resultOffset=&resultRecordCount=&sqlFormat=none&token=`;
        const urlHistory = `${this.rkiApiEndpoint}query?where=NeuerFall+IN%281%2C0%29+AND+IdBundesland=${id}+AND+MeldeDatum+%3E%3D+TIMESTAMP+%27${apiStartDate}%27&objectIds=&time=&resultType=standard&outFields=AnzahlFall%2CMeldeDatum&returnIdsOnly=false&returnUniqueIdsOnly=false&returnCountOnly=false&returnDistinctValues=false&cacheHint=false&orderByFields=MeldeDatum&groupByFieldsForStatistics=MeldeDatum&outStatistics=%5B%7B%22statisticType%22%3A%22sum%22%2C%22onStatisticField%22%3A%22AnzahlFall%22%2C%22outStatisticFieldName%22%3A%22cases%22%7D%5D%0D%0A&having=&resultOffset=&resultRecordCount=&sqlFormat=none&f=pjson&token=`
        const hist = await this.getCases(urlToday, urlHistory);
        return typeof hist === 'boolean' ? hist : IncidenceData.completeHistory(hist, CFG.def.maxShownDays + 7, new Date().setHours(0, 0, 0, 0));
    ...
    async vaccineData(): Promise<{ lastUpdate?: string, data: unknown[] } | false> {
    ...
    static isApiMetaArea(meta: unknown): meta is ApiMetaArea {
    ...
    static isApiVaccinatedAtLeastOnce(data: unknown): data is ApiVaccinatedAtLeastOnce {
        if (typeof data !== "object" || data === null) {
    ...
    static isApiVaccineFullyVaccinated(data: unknown): data is ApiVaccineFullyVaccinated {
        if (typeof data !== "object" || data === null) {
    ...
    static isApiVaccineData(data: unknown): data is ApiVaccineData {
        if (data === undefined) {
        ...
        if (!RkiService.isApiVaccinatedData<ApiVaccinatedAtLeastOnce>(data['vaccinatedAtLeastOnce'], RkiService.isApiVaccinatedAtLeastOnce)) {
        ...
        if (!RkiService.isApiVaccinatedData<ApiVaccineFullyVaccinated>(data['fullyVaccinated'], RkiService.isApiVaccineFullyVaccinated)) {
    ...
    static isApiVaccinatedData<T>(data: unknown, fn: (a: unknown) => a is T): data is ApiVaccinatedData<T> {
        if (typeof data !== 'object' || data === null) {
